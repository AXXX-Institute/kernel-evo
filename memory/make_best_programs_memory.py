...
from extract_fixes_from_logs import parse_log_file
...
def main():
    parser = argparse.ArgumentParser(
        description="Build best-programs memory from validation logs (top N per problem)."
    )
    parser.add_argument(
        "--source-dirs",
        type=str,
        nargs="+",
        required=True,
        help="Directories containing validate_logs (each has experiment subdirs with .log files)",
    )
    parser.add_argument(
        "--memory-dir",
        type=str,
        required=True,
        help="Directory for AmemGamMemory checkpoint",
    )
    parser.add_argument(
        "--top-n",
        type=int,
        default=4,
        help="Max number of best programs to keep per problem (default: 4)",
    )
    parser.add_argument(
        "--rebuild-interval",
        type=int,
        default=1000,
        help="AmemGamMemory rebuild_interval",
    )
    args = parser.parse_args()

    best = collect_best_per_problem(args.source_dirs, top_n=args.top_n)
    memory = AmemGamMemory(
        checkpoint_path=args.memory_dir, rebuild_interval=args.rebuild_interval
    )
    ...
    memory.rebuild()
    print(f"Saved {total_saved} best-program entries ({len(best)} problems) to {args.memory_dir}")
