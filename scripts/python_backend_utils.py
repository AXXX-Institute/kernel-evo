...
def build_task_description_python(
    *,
    ...
    return (
        ...
        f"- Efficient implementation primary for {run_cfg['precision']} precision, so you can use type specific optimizations\n"
        "\n"
        ...
        "- If the prompt asks for code inside a JSON field (e.g., `code`): put **only valid Python** in that field (no markdown).\n"
        "\n"
        ...
        "- Prefer fusion: fewer kernel launches, fewer intermediate tensors, fewer full passes over HBM-resident tensors.\n"
        "- For Triton: use masked `tl.load`/`tl.store` for tail tiles, prefer matmul-style blocking (`tl.dot`) for GEMM-like work,\n"
        ...
        "When asked for a justification, include a short **kernel plan**: which Torch ops were replaced, which kernels were added,\n"
